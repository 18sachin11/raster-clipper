...
if st.button("▶ Clip Rasters"):
    if not shp_upload or not tif_upload:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # c) Read shapefile once (keep its native CRS)
            ...
            with zipfile.ZipFile(zip_buf, "w") as zipf:
                ...
                for tif in tif_upload:
                    ...
                    with rasterio.open(tif_path) as src:
                        # Reproject shapefile geometries into the raster's CRS
                        shp_in_raster_crs = shapefile.to_crs(src.crs)
                        geoms = [feat.__geo_interface__ for feat in shp_in_raster_crs.geometry]
